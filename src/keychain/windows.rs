...
impl WindowsKeychain {
    ...
    #[cfg(target_os = "windows")]
    fn target_name_refresh() -> Vec<u16> {
        use std::os::windows::ffi::OsStrExt;
        let name = format!("{}:{}", SERVICE_NAME, REFRESH_TOKEN_ACCOUNT);
        std::ffi::OsStr::new(&name)
            .encode_wide()
            .chain(std::iter::once(0))
            .collect()
    ...
    #[cfg(target_os = "windows")]
    fn target_name_agent_token() -> Vec<u16> {
        use std::os::windows::ffi::OsStrExt;
        let name = format!("{}:{}", SERVICE_NAME, AGENT_TOKEN_ACCOUNT);
        std::ffi::OsStr::new(&name)
            .encode_wide()
            .chain(std::iter::once(0))
            .collect()
...
impl KeychainProvider for WindowsKeychain {
    ...
    fn save_refresh_token(&self, token: &str) -> Result<()> {
        ...
        let target_name = Self::target_name_refresh();
        let token_bytes = token.as_bytes();
    ...
    fn save_agent_token(&self, token: &str) -> Result<()> {
        ...
        let target_name = Self::target_name_agent_token();
        let token_bytes = token.as_bytes();
